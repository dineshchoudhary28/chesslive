...
module.exports = io => {
    io.on('connection', socket => {
        ...
        socket.on('joinGame', function (data) {
            ...
            } else if (game.blackName === username) {
                game.black = socket.id;
            } else if (!game.whiteName) {
                game.white = socket.id;
                game.whiteName = username;
            } else if (!game.blackName) {
                game.black = socket.id;
                game.blackName = username;
            } else {
                console.log('Room full');
            }

            if (game.whiteName && game.blackName) {
                // Both players present
                if (game.white) {
                    io.to(game.white).emit('startGame', {
                        opponent: game.blackName,
                        color: 'white'
                    });
                }
                if (game.black) {
                    io.to(game.black).emit('startGame', {
        ...
        socket.on('disconnect', function () {
            ...
            if (currentCode && games[currentCode]) {
                ...
                setTimeout(() => {
                    if (games[currentCode] && !games[currentCode].white && !games[currentCode].black) {
                        console.log('Cleaning up game', currentCode);
                        delete games[currentCode];
                    }
                }, 60000); // Remove after 1 minute if both empty
            }
        });
    });
};